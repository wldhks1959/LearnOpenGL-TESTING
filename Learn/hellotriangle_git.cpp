...
int main()
{
    ...
    /* glVertexAttribPointer(어떤 vertex attribute 설정할지 지정(layout(location = 0))으로 설정했던 것을 기억
    ,vertex attribute의 크기 지정, 데이터 유형을 지정, 데이터가 정규화되어야 하는지 지정, 스트라이드라 불리며 연속적인 
    vertex attribute 사이의 간격을 나타냄, 이상한 캐스트가 필요하다. vertex 데이터가 버퍼에서 시작하는 offset을 나타냄
    (위치 데이터가 데이터 배열의 시작에 있어서 이 값은 0이다.))
    */
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUseProgram(shaderProgram); // 객체를 redering할 때, shader program을 사용
